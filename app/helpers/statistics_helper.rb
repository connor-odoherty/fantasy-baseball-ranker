module StatisticsHelper
  ...
  def self.cell_width(attribute_name)
    raise "No cell width attributes found for field: #{attribute_name}" unless STATISTICS_MAP[attribute_name].present? && STATISTICS_MAP[attribute_name][:display_type].present?

    CELL_SIZE_MAP[self.display_type(attribute_name)]
  end
  ...
  def self.batting_fields_basic
    %i[games plate_appearances runs runs_batted_in home_runs stolen_bases batting_average]
  end
  ...
  def self.pitching_combined_overview
    (basic_pitching + expected_era_peripherals + pitching_plate_rates + pitching_control_peripherals + pitching_contact_peripherals) - [:left_on_base_rate, :bb_per_nine, :medium_contact_rate]
  end
